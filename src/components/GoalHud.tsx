...
export function GoalHud({ progress, units }: Props) {
  ...
  const { goal, distancePct, timePct, metersAhead, timeLeftMs, status, dashToFinish, remainingMeters } = progress;
  const distMarker = Math.max(0, Math.min(100, distancePct * 100));
  const timeMarker = Math.max(0, Math.min(100, timePct * 100));

  // Dash state overrides normal styling for dramatic effect.
  const isDash = dashToFinish && status !== "complete" && status !== "failed";

  const leadColor = isDash
  ...
  const leadText = isDash
    ? `${formatDistance(remainingMeters, units)} LEFT`
    : status === "complete"
    ? "GOAL HIT"
    : status === "failed"
    ? "CLOCK GONE"
  ...
  return (
    <motion.div
      animate={
        isDash
          ? { boxShadow: ["0 0 0 rgba(255,59,31,0)", "0 0 24px rgba(255,59,31,0.35)", "0 0 0 rgba(255,59,31,0)"] }
          : { boxShadow: "0 0 0 rgba(0,0,0,0)" }
      }
      transition={isDash ? { duration: 0.9, repeat: Infinity, ease: "easeInOut" } : { duration: 0.3 }}
      className={`relative overflow-hidden rounded-xl border ${
        isDash ? "border-[var(--color-blaze)]" : "border-[var(--color-line)]"
      } bg-[var(--color-ink-2)]/80 px-3 py-2.5 transition-colors sm:px-4 sm:py-3`}
    >
      {isDash && (
        <div className="mb-1 font-display text-[10px] uppercase tracking-[0.35em] text-[var(--color-blaze)]">
          ● finish line
        </div>
      )}
        ...
        <div className="relative h-3 overflow-hidden rounded-full border border-[var(--color-line)] bg-[var(--color-ink)]/70">
          <motion.div
            className={`absolute inset-y-0 left-0 ${
              isDash ? "bg-[var(--color-blaze)]" : "bg-[var(--color-volt)]/60"
            }`}
            initial={false}
